 ...
 */
public class ConvertProject31to32team {
    public static void checkTeam(File projectRootFolder) throws Exception {
        if (isSVNDirectory(projectRootFolder) || isGITDirectory(projectRootFolder)) {
            // project is 3.1-style team project
            if (isConsoleMode()) {
                Core.getMainWindow().displayWarningRB("TEAM_31_TO_32_CONSOLE");
                return;
            }

            // ask for convert
            int res = JOptionPane.showConfirmDialog(Core.getMainWindow().getApplicationFrame(),
                    OStrings.getString("TEAM_31_to_32_CONFIRM_MESSAGE"),
                    OStrings.getString("TEAM_31_to_32_CONFIRM_TITLE"), JOptionPane.OK_CANCEL_OPTION);
    ...
    /**
     * Check if project contains 3.1- style SVN directory.
     */
    ...
    /**
     * Check if project contains 3.1- style GIT directory.
     */
